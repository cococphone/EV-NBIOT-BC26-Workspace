            ...
            if (p1)
            {
				...
				if ( param_num == 3 )
				{
					...
					if ( recv_data_format == 0 )
					{
						...
						osLogI(0,"recv_data_format = %d [socket_param->buffer] %s\r\n",recv_data_format, socket_param->buffer);
					}
					else if ( recv_data_format == 1 )
					{
						Ql_strncpy(socket_param->buffer, param_list[2], socket_param->length*2 );
						osLogI(0,"recv_data_format = %d length:%d [socket_param->buffer] %s\r\n",recv_data_format,socket_param->length,socket_param->buffer);
						osLogI(0,"show recieve:%s \r\n",line);
					}
				}
            }
...
s32  RIL_SOC_QIOPEN(ST_Socket_Param_t* socket_param_t)
{
    ...
    ret = Ql_RIL_SendATCmd(strAT,Ql_strlen(strAT),NULL,NULL,SOC_TIMEOUT);
    osLogI(1,"<-- Send AT:%s, ret = %d -->\r\n",strAT, ret);
    ...
    ret = Ql_RIL_SendATCmd(strAT,Ql_strlen(strAT),ATRsp_Soc_Qiopen_Handler,&ril_socket_param,SOC_TIMEOUT);
	osLogI(1,"<-- Send AT:%s, ret = %d -->\r\n",strAT, ret);
    if(RIL_AT_SUCCESS != ret)
    {
...
s32 RIL_SOC_QISEND(u8 connectID, u32 send_length,u8* send_buffer)
{
    ...
    ret = Ql_RIL_SendATCmd(strAT,Ql_strlen(strAT),ATRsp_Soc_Qisend_Handler,0,50);
...
s32 RIL_SOC_QIRD(u8 connectID, u32 read_length,s32* recv_actual_length,s32* recv_remain_length,u8* recv_buffer)
{
    ...
    ret = Ql_RIL_SendATCmd(strAT,Ql_strlen(strAT),ATRsp_Soc_Qird_Handler,&socket_Param,50);
	
	*recv_actual_length = socket_Param.length;
	*recv_remain_length = socket_Param.remain_length;
	osLogI(0,"<--socket_Param.buffer:%s-->\r\n",socket_Param.buffer);
...

{
    ...
    ret = Ql_RIL_SendATCmd(strAT,Ql_strlen(strAT),ATResponse_Handler,0,SOC_TIMEOUT);
...
s32 RIL_SOC_QICLOSE(u8 connectID)
{
    ...
    ret = Ql_RIL_SendATCmd(strAT,Ql_strlen(strAT),ATRsp_Soc_Qiclose_Handler,0,SOC_TIMEOUT);
